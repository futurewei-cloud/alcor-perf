...
#                    name="AlcorRouterPerformancePlugin.alcor_router_scalability") 
@scenario.configure(context={"cleanup@openstack": ["neutron", "nova"]}, name="AlcorRouterPerformancePlugin.alcor_router_scalability")
#class AlcorRouterPerformancePlugin(utils.NeutronScenario, nova_utils.NovaScenario, cinder_utils.CinderScenario):  
class AlcorRouterPerformancePlugin(utils.NeutronScenario, nova_utils.NovaScenario):

    def run(self, image, flavor, detailed=True, 
            network_create_args=None, subnet_create_args=None, port_create_args=None,
            subnet_cidr_start=None, subnets_per_network=2, router_create_args=None, instances_per_network=1, 
            ports_per_instance=1, force_delete=1, **kwargs):
            ...
            servers = self._boot_10_servers(image, flavor, 1, instances_amount=instances_per_network, **kargs)
            ...
            for server in servers:
                ...
                for j in range(ports_per_instance):
                    ...
                    LOG.info("*** Got port id: %s", port["port"]["id"]);
