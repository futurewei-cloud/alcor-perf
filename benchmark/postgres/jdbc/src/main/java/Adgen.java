...
public class Adgen
{
    ...
    public static String ADGENUsage = "\n" +
            ...
            "adgen -s schema_file [-i indelim] [-o outdelim] [-n numrec] [-m] [-b batch_size] [-u url] [-U username] [-p password] [-D datasource name] [-t tablename1,tablename2...]\n" +
            "\n" +
            ...
            "-u url     -> Use url as JDBC connection string\n" +
            "-U username    -> Database username\n" +
            "-p password    -> Database user password\n" +
            "-t tablename1,tablename2...    -> read definitions from datasource\n" +
            "              specific versions, tablenameN-datasource-ddl.sql,\n" +
            "              or generic tablenameN-ddl.sql, and query from\n" +
            "              datasource-dml.sql, or query-dml.sql\n" +
            ";";
...
/*
        tdef = new TableDef();
        tdef.tblName = "nodeinfo";
        tdef.columns = new ArrayList<>();
        tdef.columns.add(new ColumnDef("node_id", AclorType.AT_STRING, "-1"));

        tdef.columns.add(new ColumnDef("node_name", AclorType.AT_STRING, "-1"));

        tdef.columns.add(new ColumnDef("local_ip", AclorType.AT_STRING, "40"));

        tdef.columns.add(new ColumnDef("mac_address", AclorType.AT_STRING, "40"));

        tdef.columns.add(new ColumnDef("veth", AclorType.AT_INT, "-1"));

        tdef.columns.add(new ColumnDef("host_dvr_mac", AclorType.AT_STRING, "18"));
    ...
    public static void processArgs(String[] args, CommandLine cmdLine)
    {
        ...
        for (int i = 0; i < args.length; ++i) {
            ...
            if (args[i].startsWith("-D")) {
                ...
                System.out.println("-t is not implemented yet");
                System.exit(-1);
            }
            else {
    ...
    public static void genData(TableDef tdef, DataBuffer dbuf, int numRows) throws Exception
    {
        /**
